...
export default function TareasPage() {
  ...
  return (
    <div className="space-y-4 fade-up">
      ...
      <div className="flex flex-wrap gap-2 items-end bg-white p-3 border rounded-xl">
        <Field label="Buscar">
          <input value={q} onChange={(e) => setQ(e.target.value)} className="border rounded px-2 py-1" placeholder="descripción, equipo…" />
        </Field>
        <Field label="ABC">
          <select value={abc} onChange={(e) => setAbc(e.target.value)} className="border rounded px-2 py-1">
        ...
        <Field label="Año">
          <input type="number" min={2000} max={2100} value={year} onChange={(e) => setYear(e.target.value ? Number(e.target.value) : '')} className="border rounded px-2 py-1 w-24" />
        </Field>
        <Field label="Mes">
          <input type="number" min={1} max={12} value={month} onChange={(e) => setMonth(e.target.value ? Number(e.target.value) : '')} className="border rounded px-2 py-1 w-20" />
        </Field>
        <span className="text-sm text-slate-500 ml-auto">{data?.total ?? 0} resultados</span>
      </div>
          ...
          <tbody>
            ...
            {!isLoading && !isError && data?.rows.map((t) => (
              <tr key={t.id} className="border-t hover:bg-slate-50">
                <td className="px-3 py-2 font-medium">{t.indicadorAbc ?? '—'}</td>
